...
c = p.loadURDF('combined/aruco.urdf', cam_pos, useFixedBase=True)
...
pos = p.calculateInverseKinematics(boxId, eefLinkIdx, cam_pos, maxNumIterations=100)
...
p.setJointMotorControlArray(bodyIndex=boxId,
                            jointIndices=jointIndices,
                            targetPositions=[pos[0], pos[1], 0.5],
                            controlMode=p.POSITION_CONTROL)
for _ in range(100):
    p.stepSimulation()
